    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a EuropeanPropertyCoin address (e.g. 5PFJ2cDa7x29ZSB2mGM5mP2rb69FbNK9T8)"));
#endif
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid EuropeanPropertyCoin address"));
        }
        else
        {
...
{qDebug() << "In";
    ...
    {
        QMessageBox::warning(this, tr("Anonymizing Sending of EuropeanPropertyCoins"),
            tr("Error: %1").
...
{qDebug() << "Out";
    ...
    QMessageBox::StandardButton retval = QMessageBox::question(this, tr("Confirm Send"),
                          tr("Do you wish to confirm anonymous sending of EuropeanPropertyCoins? Total EuropeanPropertyCoins with tx fee required is %1.").arg(formatted.join(tr(" and "))),
          QMessageBox::Yes|QMessageBox::Cancel,
          QMessageBox::Cancel);
