...
void EnsureWalletIsUnlocked()
{
    if (pwalletMain->IsLocked())
        throw JSONREPCrror(RPC_WALLET_UNLOCK_NEEDED, "Error: Please enter the wallet passphrase with walletpassphrase first.");
    if (fWalletUnlockStakingOnly)
        throw JSONREPCrror(RPC_WALLET_UNLOCK_NEEDED, "Error: Wallet is unlocked for staking only.");
}
...
string AccountFromValue(const Value& value)
{
    ...
    if (strAccount == "*")
        throw JSONREPCrror(RPC_WALLET_INVALID_ACCOUNT_NAME, "Invalid account name");
    return strAccount;
}
...
Value getnewpubkey(const Array& params, bool fHelp)
{
    ...
    if (!pwalletMain->GetKeyFromPool(newKey, false))
        throw JSONREPCrror(RPC_WALLET_KEYPOOL_RAN_OUT, "Error: Keypool ran out, please call keypoolrefill first");
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new EuropeanPropertyCoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current EuropeanPropertyCoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <EuropeanPropertyCoinaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONREPCrror(RPC_INVALID_ADDRESS_OR_KEY, "Invalid EuropeanPropertyCoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount <EuropeanPropertyCoinaddress>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <EuropeanPropertyCoinaddress> <amount> [narration] [comment] [comment-to]\n"
    ...
    if (strError != "")
        throw JSONREPCrror(RPC_WALLET_ERROR, strError);
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <EuropeanPropertyCoinaddress> <message>\n"
    ...
    if (!addr.IsValid())
        throw JSONREPCrror(RPC_TYPE_ERROR, "Invalid address");

    CKeyID keyID;
    if (!addr.GetKeyID(keyID))
        throw JSONREPCrror(RPC_TYPE_ERROR, "Address does not refer to key");

    CKey key;
    if (!pwalletMain->GetKey(keyID, key))
        throw JSONREPCrror(RPC_WALLET_ERROR, "Private key not available");
    ...
    if (!key.SignCompact(Hash(ss.begin(), ss.end()), vchSig))
        throw JSONREPCrror(RPC_INVALID_ADDRESS_OR_KEY, "Sign failed");
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <EuropeanPropertyCoinaddress> <signature> <message>\n"
    ...
    if (fInvalid)
        throw JSONREPCrror(RPC_INVALID_ADDRESS_OR_KEY, "Malformed base64 encoding");
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <EuropeanPropertyCoinaddress> [minconf=1]\n"
            "Returns the total amount received by <EuropeanPropertyCoinaddress> in transactions with at least [minconf] confirmations.");
...
Value movecmd(const Array& params, bool fHelp)
{
    ...
    if (!walletdb.TxnBegin())
        throw JSONREPCrror(RPC_DATABASE_ERROR, "database error");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <toEuropeanPropertyCoinaddress> <amount> [minconf=1] [narration] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
    ...
    int64_t nBalance = GetAccountBalance(strAccount, nMinDepth);
    if (nAmount > nBalance)
        throw JSONREPCrror(RPC_WALLET_INSUFFICIENT_FUNDS, "Account has insufficient funds");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONREPCrror(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid EuropeanPropertyCoin address: ")+s.name_);

        if (setAddress.count(address))
            throw JSONREPCrror(RPC_INVALID_PARAMETER, string("Invalid parameter, duplicated address: ")+s.name_);
        setAddress.insert(address);
    ...
    if (!fCreated)
    {
        if (totalAmount + nFeeRequired > pwalletMain->GetBalance())
            throw JSONREPCrror(RPC_WALLET_INSUFFICIENT_FUNDS, "Insufficient funds");
        throw JSONREPCrror(RPC_WALLET_ERROR, "Transaction creation failed");
    }
    if (!pwalletMain->CommitTransaction(wtx, keyChange))
        throw JSONREPCrror(RPC_WALLET_ERROR, "Transaction commit failed");
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a EuropeanPropertyCoin address or hex-encoded public key\n"
...
Value listtransactions(const Array& params, bool fHelp)
{
    ...
    if (nCount < 0)
        throw JSONREPCrror(RPC_INVALID_PARAMETER, "Negative count");
    if (nFrom < 0)
        throw JSONREPCrror(RPC_INVALID_PARAMETER, "Negative from");
...
Value gettransaction(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        if (GetTransaction(hash, tx, hashBlock))
        {
            ...
            throw JSONREPCrror(RPC_INVALID_ADDRESS_OR_KEY, "No information available about transaction");
    }
...
Value backupwallet(const Array& params, bool fHelp)
{
    ...
    if (!BackupWallet(*pwalletMain, strDest))
        throw JSONREPCrror(RPC_WALLET_ERROR, "Error: Wallet backup failed!");
...
Value keypoolrefill(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0) {
        if (params[0].get_int() < 0)
            throw JSONREPCrror(RPC_INVALID_PARAMETER, "Invalid parameter, expected valid size");
    ...
    if (pwalletMain->GetKeyPoolSize() < nSize)
        throw JSONREPCrror(RPC_WALLET_ERROR, "Error refreshing keypool.");
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("EuropeanPropertyCoin-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("EuropeanPropertyCoin-lock-wa");
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONREPCrror(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletpassphrase was called.");

    if (!pwalletMain->IsLocked())
        throw JSONREPCrror(RPC_WALLET_ALREADY_UNLOCKED, "Error: Wallet is already unlocked, use walletlock first if need to change unlock settings.");
    ...
    if (strWalletPass.length() > 0)
    {
        if (!pwalletMain->Unlock(strWalletPass))
            throw JSONREPCrror(RPC_WALLET_PASSPHRASE_INCORRECT, "Error: The wallet passphrase entered was incorrect.");
    }
    else
        throw runtime_error(
...
Value walletpassphrasechange(const Array& params, bool fHelp)
{
    ...
    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONREPCrror(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletpassphrasechange was called.");
...
Value walletlock(const Array& params, bool fHelp)
{
    ...
    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONREPCrror(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletlock was called.");
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    if (fHelp)
        return true;
    if (pwalletMain->IsCrypted())
        throw JSONREPCrror(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an encrypted wallet, but encryptwallet was called.");
    ...
    if (!pwalletMain->EncryptWallet(strWalletPass))
        throw JSONREPCrror(RPC_WALLET_ENCRYPTION_FAILED, "Error: Failed to encrypt the wallet.");

    // BDB seems to have a bad habit of writing old data into
    // slack space in .dat files; that is bad if the old data is
    // unencrypted private keys. So:
    StartShutdown();
    return "wallet encrypted; EuropeanPropertyCoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <EuropeanPropertyCoinaddress>\n"
            "Return information about <EuropeanPropertyCoinaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <EuropeanPropertyCoinpubkey>\n"
            "Return information about <EuropeanPropertyCoinpubkey>.");
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new EuropeanPropertyCoin stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid EuropeanPropertyCoin stealth address."));
        return result;
    };
    ...
    if (!pwalletMain->SendStealthMoneyToDestination(sxAddr, nAmount, sNarr, wtx, sError))
        throw JSONREPCrror(RPC_WALLET_ERROR, sError);
