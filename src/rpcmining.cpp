...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONREPCrror(-9, "EuropeanPropertyCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONREPCrror(-10, "EuropeanPropertyCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONREPCrror(RPC_CLIENT_NOT_CONNECTED, "EuropeanPropertyCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONREPCrror(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "EuropeanPropertyCoin is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0)
    {
        ...
        else if (modeval.type() == null_type)
        {
            ...
            throw JSONREPCrror(RPC_INVALID_PARAMETER, "Invalid mode");
    }
